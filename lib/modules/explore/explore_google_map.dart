...
import 'package:Happinest/modules/explore/explore_controller.dart';
...
class ExploreMap extends ConsumerStatefulWidget {
  const ExploreMap({super.key, required this.stories});
  final List<Stories> stories;
  @override
  ConsumerState<ExploreMap> createState() => MapScreenState();
}

class MapScreenState extends ConsumerState<ExploreMap> {
  GoogleMapController? _mapController;

  LatLng _initialPosition = const LatLng(45.4380528374478, 12.3359325528145); // San Francisco, CA
  ...
  Future _addMarkers(Stories data, int i) async {
    ...
    if (lat != null && long != null && lat != 'null' && long != 'null') {
      ...
      //         final _ = ref.read(exploreCtr);
      //         _.currPage = index;
      //         _.pageController.animateToPage(index, duration: Duration(milliseconds: 500), curve: Curves.easeInOut);
      //         _.notifyListeners();
      //         },
      //     ),
      //   );
  ...
  Widget build(BuildContext context) {
    ...
    if (isMarkerGenerated) {
      if (dayID != _.currPage && haveMarkers[_.currPage]) {
        Stories data = widget.stories[_.currPage];
        int index =
