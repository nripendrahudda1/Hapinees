...
class ProfilePostMap extends ConsumerStatefulWidget {
  const ProfilePostMap({super.key, required this.stories});
  final List<Stories> stories;
  @override
  ConsumerState<ProfilePostMap> createState() => MapScreenState();
}

class MapScreenState extends ConsumerState<ProfilePostMap> {
  ...
  Future _addMarkers(Stories data, int i) async {
    ...
    if (lat != null && long != null && lat != 'null' && long != 'null') {
      ...
      await getBytesFromNetworkImage(data.backgroundImageUrl ?? '',
  ...
  Widget build(BuildContext context) {
    ...
    if (isMarkerGenerated) {
      if (dayID != _.currFavPostPage && haveMarkers[_.currFavPostPage]) {
        Stories data = widget.stories[_.currFavPostPage];
