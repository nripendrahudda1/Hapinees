...
class ProfileAuthorMap extends ConsumerStatefulWidget {
  const ProfileAuthorMap({super.key, required this.stories});
  final List<Authors> stories;
  @override
  ConsumerState<ProfileAuthorMap> createState() => MapScreenState();
}

class MapScreenState extends ConsumerState<ProfileAuthorMap> {
  ...
  Future _addMarkers(Authors data, int index) async {
    var lat = data.storyLat;
    var long = data.storyLong;
  ...
  void _fitMarkers(List<Marker> mrkr) async {
    ...
    bounds != null
      ...
      CameraUpdate.newLatLngBounds(
        extendBoundsByDistance(bounds!, 1),
        80,
      ),
    )
        : null;
  ...
  Widget build(BuildContext context) {
    ...
    //     dayID = _.currPage;
    //     _fitMarkers([_markers[_.currPage]]);
    //   }
    // }
