...
class ProfileEventMap extends ConsumerStatefulWidget {
  const ProfileEventMap({super.key, required this.stories});
  final List<Stories> stories;
  @override
  ConsumerState<ProfileEventMap> createState() => MapScreenState();
}

class MapScreenState extends ConsumerState<ProfileEventMap> {
  GoogleMapController? _mapController;

  LatLng _initialPosition = const LatLng(51.6744575, 0.4472569); // San Francisco, CA
  ...
  Future<Uint8List> getBytesFromNetworkImage(String imageUrl, int width, int height) async {
    try {
      Dio dio = Dio();
      ...
      if (response.statusCode == 200) {
        final ByteData data = ByteData.view(Uint8List.fromList(response.data!).buffer);
  ...
  //   var long =
  //       data.storyLong.toString() == "string" || data.storyLong == "0.0" ? null : data.storyLong;
  //   // print("_addMarkers ${lat}  ${long}");
  ...
  //       });
  //     }
  //     await getBytesFromNetworkImage(
  //             "https://test.traveloryapp.com/traveloryimages/StoryMarker/Travel_Marker.png",
  //             125,
  //             155)
  //         .then((markerIcon) {
  //       _markers.add(
  ...
  Future _addMarkers(Stories data, int i) async {
    ...
    if (lat != null && long != null && lat != 'null' && long != 'null') {
      ...
      final icon = await getBitmapDescriptorFromAssetBytes(TImageName.eventMarker, 120);
      _markers.add(
        Marker(
          markerId: MarkerId(data.storyId.toString()),
          position: LatLng(lati, longi),
  ...
  LatLngBounds extendBoundsByDistance(LatLngBounds bounds, double distanceInKm) {
    ...
    double lonExtension = distanceInKm / (degreeToKm * cos(pi * bounds.northeast.latitude / 180.0));
  ...
  Widget build(BuildContext context) {
    ...
    if (isMarkerGenerated) {
      if (dayID != _.currEventPage && haveMarkers[_.currEventPage]) {
        Stories data = widget.stories[_.currEventPage];
        int index =
            _markers.indexWhere((element) => element.markerId.value == data.storyId.toString());
        dayID = index;
